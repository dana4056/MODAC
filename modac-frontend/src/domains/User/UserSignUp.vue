    ...
    <div :class="$style.login_left_flex">
      <div :class="$style.login_div_flex">
        <div :class="$style.login_description">
          ...
          <h1 :class="$style.login_title"><span :class="$style.text_green"> { 모닥 }</span>이 처음이신가요?</h1>
          <p :class="$style.login_p">지금도 늦지 않았어요! <span :class="$style.text_green"> { MODAC }</span>에서 공부와 기록을 도와드릴게요.</p>
        </div>
        <form id="loginform" :class="$style.login_form">
          <div class="flex flex-col items-end">
            <input :class="$style.login_form_input" 
                    type="text" 
                    v-model="user_id" 
                    id="user_id" 
                    ...
                    :onkeyup="checkByte"
                    required>
                    <div class="block text-xs">(<span id="user_id_byte">0</span>/20bytes)</div>
          </div>
<!-- 
          <div>
            <input type="text" placeholder="닉네임을 입력하세요" :class="$style.login_form_input">
          </div> -->
          ...
          <div class="flex flex-col items-end">
            <input :class="$style.login_form_input" 
                    ...
                    :onkeyup="checkByte"
                    required>
                    <div class="flex text-xs">(<span id="user_nickname_byte">0</span>/20bytes)</div>
          </div>
          ...
          <div>
            ...
            <p :class="$style.update_password_p">
              비밀번호는 최소 8자리에서 최대 16자리까지 숫자, 영문 소문자, 대문자, 특수문자 각 1개 이상 포함하여 작성하세요.
            </p>
          </div>
...
<!-- 
          <div>
            <input type="text" placeholder="이메일을 입력하세요" :class="$style.login_form_input">
          </div> -->
          ...
          <button v-if="user_signup_check_id && user_signup_check_nickname && (checkCode === 0 ? true : false) && (user_password_check === user_password ? true : false)"
                  type="submit" 
                  id="signup-button"
                  :class="$style.login_form_login_button" 
                  @click="userSignUp">회원가입</button>
          <button v-else
                  type="submit" 
                  id="signup-button"
                  :class="$style.login_form_login_button" 
...
let length = computed(() => {
  return (8 <= user_password.value.length 
          && user_password.value.length <= 16) ? true : false
})
...
let checkMessage2 = computed(() => {
  ...
  if (user_password_check.value !== user_password.value) return "비밀번호 확인은 새 비밀번호와 동일하게 입력해주세요."
  return "확인이 완료되었습니다."
})

const update_password_check_input_css = computed(() => {
  if (user_password_check.value === "") return "update_password_input";
  if (user_password_check.value === user_password.value) return "update_password_input_success";
  return "update_password_input_fail";
})

const checkMessage2_css = computed(() => {
  if (user_password_check.value === "") return "update_password_p";
  if (user_password_check.value === user_password.value) return "update_password_p_success";
  return "update_password_p_fail";
})
...
const checkByte = (obj) => {
  ...
  if(obj.target.id == "user_password_input" || obj.target.id == "user_password_check_input") maxByte = 16;
  ...
  if(totalByte > maxByte){
    if(obj.target.id == "user_password_input" || obj.target.id == "user_password_check_input") alert('최대 16Byte까지만 입력가능합니다.');
    else alert('최대 20Byte까지만 입력가능합니다.');

    if(obj.target.id == "user_id") user_signup_check_id.value = false;
    else if(obj.target.id == "user_nickname") user_signup_check_nickname.value = false;
    else if(obj.target.id == "user_password_input") user_signup_check_pass.value = false;
    else if(obj.target.id == "user_password_check_input") user_signup_check_pass_check.value = false;

    document.getElementById(obj.target.id + "_byte").innerText = totalByte;
    document.getElementById(obj.target.id + "_byte").style.color = "rgb(248 113 113)";
  } else {
    ...
    document.getElementById(obj.target.id + "_byte").innerText = totalByte;
    document.getElementById(obj.target.id + "_byte").style.color = "rgb(134 239 172)";
  }
}

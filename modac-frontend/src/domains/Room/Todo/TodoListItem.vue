<template lang="">
  <div style="display: flex;
    ...
    <div style="width:90%; flex-direction: column;" class="gap-2 flex" :id="`todo_contents_${props.todo.seq}`">
      ...
      <div v-if="!ellipsisState" @click="showFullTodo" class="break-all whitespace-normal h-full">  {{ props.todo.title }}  </div>

      <div class="flex items-center justify-between">
        <div class="flex">
          <div>
            <span v-if="props.todo.categories_seq == 0" class="bg-blue-100 text-blue-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded-full dark:bg-blue-900 dark:text-blue-300">#알고리즘</span>
            <span v-if="props.todo.categories_seq == 1" class="bg-yellow-100 text-yellow-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded-full dark:bg-yellow-900 dark:text-yellow-300">#CS</span>
            <span v-if="props.todo.categories_seq == 2" class="bg-red-100 text-red-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded-full dark:bg-red-900 dark:text-red-300">#개발</span>
            <span v-if="props.todo.categories_seq == 3" class="bg-green-100 text-green-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded-full dark:bg-green-900 dark:text-green-300">#기획</span>
            <span v-if="props.todo.categories_seq == 4" class="bg-gray-100 text-gray-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded-full dark:bg-gray-700 dark:text-gray-300">#기타</span>
          </div>
          
          <!-- 타이머 -->
          <div id="clock">
            <span v-if="props.todo.status == 0" class="bg-indigo-100 text-indigo-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-indigo-400 border border-indigo-400">{{ time }}</span>
            <span v-if="props.todo.status == 1" class="bg-red-100 text-red-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-red-400 border border-red-400">{{ time }}</span>
            <span v-if="props.todo.status == 2" class="bg-gray-100 text-gray-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-gray-400 border border-gray-500">{{ time }}</span>
      ...
      <div v-if="updateState">
        <hr class="bg-white p-0 my-5 border-black border-0 border-t border-dashed">
        <div class="my-5">
          <form @submit.prevent="updateTodo">

            <div class="w-full gap-7 max-w-sm p-4 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
              <!-- 할 일 작성 -->
              <div class="pb-3 m-0.5 w-full">
                <label class="block mb-2 text-sm font-semibold text-gray-900 dark:text-white">할 일</label>
                <input type="text" v-model="updateTodoItem.title" id="title" autofocus class="bg-gray-50 text-sm border border-gray-300 text-gray-900 rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="오늘의 공부는?" required>
              </div>

              <!-- 카테고리 선택 -->
              <div class="pb-3 m-0.5 w-full">
                <label class="block mb-2 text-sm font-semibold text-gray-900 dark:text-white">카테고리 선택</label>
                <select id="categories_seq" 
              ...
              <div class="inline-flex w-full">
                ...
                <div class="flex items-center justify-between w-full">
                  <button @click="openUpdateTodo" class="w-full text-sm text-black bg-gray-200 hover:bg-gray-300 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg px-4 py-2.5 m-1 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                    ❌ 취소
                  </button>
                </div>
                <!-- 수정 button -->
                <div class="flex items-center justify-between w-full">
                  <button type="submit" class="w-full text-sm text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg px-4 py-2.5 m-1 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                    ✔ 수정
                  </button>
                </div>
              </div>

              ...
              <div :class="$style.modal_title_img">
                ...
                <div class="flex justify-between flex-wrap" style="background-image: linear-gradient( rgba(0, 0, 0, 0), rgba(0, 0, 0, 1) )">
                  <div :class="$style.modal_title_info_left">
                    ...
                    <div>
                    {{ feeds[feedModalSeq].user.nickname }}
                    </div>
                  ...
                  <div :class="$style.modal_title_info_right">
                    ...
                    <div :class="$style.modal_icon">
                      ...
                      {{ feeds[feedModalSeq].commentCount }}
                    </div>
                    <div :class="$style.section_divider">|</div>
                    <div>
                      {{ feeds[feedModalSeq].registeredTime }}
                    </div>
                    <div :class="$style.section_divider">|</div>
                    <div>
                      <span v-if="feeds[feedModalSeq].categoryName === '알고리즘'" :class="$style.article_table_td_category_algorithm">#알고리즘</span>
                      <span v-if="feeds[feedModalSeq].categoryName === 'CS'" :class="$style.article_table_td_category_cs">#CS</span>
                      <span v-if="feeds[feedModalSeq].categoryName === '개발'" :class="$style.article_table_td_category_programming">#개발</span>
                      <span v-if="feeds[feedModalSeq].categoryName === '면접'" :class="$style.article_table_td_category_planning">#면접</span>
                      <span v-if="feeds[feedModalSeq].categoryName === '공통'" :class="$style.article_table_td_category_ect">#공통</span>
                    </div>
                  </div>
                </div>
              </div>
...
const feeds = articles.value.articleInfoList;

// store에서 불러 와야 할 아이들
const computedFeedList = computed(() => {
  // return useFeedStore.feeds;
